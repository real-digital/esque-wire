...
partitionErrorSerializers: Dict[int, DataClassSerializer[PartitionError]] = {
    version: DataClassSerializer(PartitionError, schema) for version, schema
    in partitionErrorSchemas.items()
}
...
errorSerializers: Dict[int, DataClassSerializer[Error]] = {
    version: DataClassSerializer(Error, schema) for version, schema
    in errorSchemas.items()
}
...
addPartitionsToTxnResponseDataSerializers: Dict[int, DataClassSerializer[AddPartitionsToTxnResponseData]] = {
    version: DataClassSerializer(AddPartitionsToTxnResponseData, schema) for version, schema
    in addPartitionsToTxnResponseDataSchemas.items()
}
