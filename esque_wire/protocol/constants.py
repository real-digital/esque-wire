...
_ERROR_CODE_METADATA = {
    ...
    2: (True, ('This message has failed its CRC checksum, exceeds the valid size, has a null key for a compacted ' 'topic, or is otherwise corrupt.'), ),
    ...
    23: (False, ("The group member's supported protocols are incompatible with those of existing members or first " 'group member tried to join with empty protocol type or empty protocol list.'), ),
    24: (False, 'The configured groupId is invalid.', ),
    25: (False, 'The coordinator is not aware of this member.', ),
    26: (False, ('The session timeout is not within the range allowed by the broker (as configured by ' 'group.min.session.timeout.ms and group.max.session.timeout.ms).'), ),
    ...
    42: (False, ('This most likely occurs because of a request being malformed by the client library or the message ' 'was sent to an incompatible broker. See the broker logs for more details.'), ),
    ...
    47: (False, ('Producer attempted an operation with an old epoch. Either there is a newer producer with the same ' "transactionalId, or the producer's transaction has been expired by the broker."), ),
    ...
    50: (False, ('The transaction timeout is larger than the maximum value allowed by the broker (as configured by ' 'transaction.max.timeout.ms).'), ),
    51: (False, ('The producer attempted to update a transaction while another concurrent operation on the same ' 'transaction was ongoing.'), ),
    52: (False, ('Indicates that the transaction coordinator sending a WriteTxnMarker is no longer the current ' 'coordinator for a given producer.'), ),
    53: (False, 'Transactional Id authorization failed.', ),
    54: (False, 'Security features are disabled.', ),
    55: (False, ('The broker did not attempt to execute this operation. This may happen for batched RPCs where some ' 'operations in the batch failed, causing the broker to respond without trying the rest.'), ),
    ...
    64: (False, ('Delegation Token requests are not allowed on PLAINTEXT/1-way SSL channels and on delegation token ' 'authenticated channels.'), ),
    ...
    72: (True, ('There is no listener on the leader broker that matches the listener on which metadata request was ' 'processed.'), ),
    ...
    78: (True, ('The leader high watermark has not caught up from a recent leader election so the offsets cannot be ' 'guaranteed to be monotonically increasing'), ),
    ...
    81: (False, ('Consumer group The consumer group has reached its max size. already has the configured maximum ' 'number of members.'), ),
    82: (False, ('The broker rejected this static consumer since another consumer with the same group.instance.id has ' 'registered with a different member.id.'), ),
}
...
class ErrorCode(Enum):
    ...
    def __init__(self, value):
        ...
        retryable, default_message = _ERROR_CODE_METADATA[value]
